"""Tests for soulcraft.synthesis — SOUL.md template synthesis and helpers."""

import pytest
from unittest.mock import patch, MagicMock

from soulcraft.synthesis import (
...
class TestTemplateSynthesize:
    ...
    def test_includes_footer(self):
        ...
        graph = _make_graph()
        result = _template_synthesize(entities, graph)
        assert "Generated by Soulcraft" in result
...
class TestSynthesizeSoulmd:
    @patch("soulcraft.synthesis.llm_available", return_value=False)
    ...
    @patch("soulcraft.synthesis.llm_available", return_value=True)
    @patch("soulcraft.synthesis._llm_synthesize")
...
class TestSynthesizeSoulmdStream:
    ...
    @patch("soulcraft.synthesis.llm_available", return_value=True)
    @patch("soulcraft.synthesis.chat_stream")
