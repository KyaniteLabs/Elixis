...
def synthesize_soulmd_stream(entities, graph):
    ...
    for event in chat_stream(messages):
        if event["type"] == "thinking":
            ...
            save_run("", entities, graph, result, stage_timings={"stage3_synthesis_ms": event.get("latency_ms", 0)})
            yield {"type": "soulmd_done", "data": {"length": len(result), "source": "llm", "latency_ms": event.get("latency_ms", 0)}}
